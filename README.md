...
설치할 것 없이 위 주소를 열고 `location-history.json`을 끌어다 놓으면 됩니다. 네트워크로 오가는 것은 페이지와 경계 데이터를 내려받는 것뿐이며, 위치 기록 파일 자체는 브라우저 밖으로 나가지 않습니다.
...
기록 파일이 없어도 **샘플 데이터로 둘러보기** 버튼으로 지어낸 2년치를 얹어 볼 수 있습니다. 영문판은 [/en/](https://taekie.github.io/timeline-trips/en/)에 있습니다.
...
기준 거리와 무관하게, 되풀이해 들른 곳을 따로 셉니다. 묶는 단위는 구글이 붙인 `placeID`입니다 — 행정구역으로 묶으면 어디인지 알 수 없는 덩어리가 되고, 좌표 격자로 묶으면 같은 건물이 갈라집니다.

한 번에 30분을 넘겼으면 **방문**, 그 아래면 **경유**로 나눕니다. 신호등에 걸린 사거리가 단골 가게보다 위로 올라오지 않게 하기 위해서입니다. 장소를 누르면 그곳에 간 날의 이동 경로가 **갈 때**(파랑)와 **올 때**(주황)로 나뉘어 깔립니다.
...
sample.json        기록이 없어도 둘러볼 수 있게 지어낸 2년치
...
지명은 한글·영문 두 벌로 들어 있습니다 — `[한글, 영문, 링(, 국가코드)]`. 영문판은 배포할 때 `i18n/apply.py`가 찍어 냅니다.
