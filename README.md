...
외부로 나가는 요청은 여행 상세의 **배경 지도** 하나뿐입니다. CARTO에서 지도 타일을 받아오며, 이때 보고 있는 지역의 타일 좌표가 그 서버에 전달됩니다. 모달의 `배경 지도` 버튼으로 끄면 타일 없이 경계 데이터만으로 그리고, 선택은 브라우저에 기억됩니다. 위치 기록 파일 자체는 어느 경우에도 브라우저 밖으로 나가지 않습니다.
